...
### 13.5 严格受控 follow-up 与新停止门

13.4 的 8wide aggregate feasibility 随后按计划进行了受控 feature/probe 复核。新门不再
允许总功率抵消逐 band 错误，并明确要求 probe 翻倍与 feature realization 稳定。结果显示：

- 同 probe、不同 reduced-feature seed 的 pure/current change 为 `0.410/0.412`；
- 同 feature、不同 probe seed 为 `0.817/0.795`；
- 16→32 probes 仍为 `0.669/0.634`；
- 固定 shrinkage 后仍约 `0.69--0.79`，排除 selector 跳变；
- 32-probe pure/current L2 为 `0.550/0.582`，没有收敛趋势。

因此 13.4 的 aggregate PASS 被撤销，且按门停止 64/128 probes、feature 扩维与 16wide。
这不是计算资源不足，而是预先声明的稳定性条件没有满足。

同时完成了 foreground mean 路线。原 estimator 对零 reference、90% amplitude 和 ±10%
linear tilt 灾难性失败；flat-prior template T0/T1 mean profile 能使四种 reference 数值完全
一致，在旧 view12 保持 pure/current L2 `0.247/0.275`。它解决 reference robustness，
但受控三视图最大逐 band 偏差仍 `0.488/0.771`、MC SE `0.589`，故不能晋级。

后续计划调整为方法族切换，而不是继续增加相同随机 probes：

1. fixed-template T0/T1 profile 只保留为 reference-robustness 诊断，不再作为默认 mean；
2. 用无固定空间模板的 analytic/deterministic covariance action 替换逐 source sample covariance，或实现
   真正 matrix-free full-plane $B S_b B^\dagger$；
3. 新候选先复用本节 same-probe/different-feature、same-feature/different-probe 和 probe
   doubling gates；
4. 只有三项受控变化均 `<=0.15` 且逐 band screen 通过，才恢复 16wide、noise split 和
   cross-power 阶段。

完整结果见
[`ps2d_v2_estimator_validation_results.md`](ps2d_v2_estimator_validation_results.md)。

### 13.6 无模板 sky-smooth flat-projection screen

随后先对最简单的无模板候选做了 8wide 无噪声可辨识性测试。前景 nuisance 写为

$$
f(\nu,x,y)=\sum_m q_m(\nu)A_m(x,y),
$$

其中每个正交 Chebyshev 谱基列都有一张完全自由的 `512×512` sky coefficient map；没有
foreground reference 或固定 morphology。测试中的 T0 和 T0/T1 仅表示常数与线性谱列，
不是旧 fixed-template profile。求解器只接收 foreground 与 EoR 相加后的 observed dirty
cube，truth components 只在求解结束后用于 transfer 和 PS2D 诊断。

这些 sky maps 经 cached stride4/rank64 operator 后，由 matrix-free LSQR 在完整 dirty cube
上做 identity-weighted flat-prior 拟合。formal operator/adjoint dot-product error 对 T0 和
T0/T1 分别为 `0` 与 `4.24e-16`。两者的系数/数据维数比为 `0.5/1.0`，因此完整 T0/T1
subspace 没有维数上的 EoR null-space 保证。

| 谱空间 | observed iterations | data residual | observed target L2 | observed power ratio | pure-EoR target L2 | pure-EoR power ratio |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| T0 | 24 | 0.08531 | `7.692e5` | `1.179e6` | 0.03153 | 0.97143 |
| T0/T1 | 24 | 0.05201 | `2.136e5` | `4.584e5` | 0.12452 | 0.88515 |
| T0 | 96 | 0.03787 | `1.699e5` | `3.144e5` | 未运行 | 未运行 |
| T0/T1 | 96 | 0.01971 | `9.002e4` | `1.276e5` | 未运行 | 未运行 |

T0/T1 在 24 次时已损失 `11.5%` 的目标区总 EoR 功率，最差 target band 损失
`12.47%`，但 residual foreground 仍高出可用水平五个以上数量级。增加到 96 次不能弥合
该差距。8-probe Hutchinson Jacobi 诊断还出现约 `47.6%` 非正或非有限对角估计，且没有
实质收敛收益，因此不保留为默认 solver。该实现没有随机 reduced features 或 sample
covariance，故不存在通过更换 feature/probe seed 获得另一个 estimator view 的问题。

停止 identity-weighted flat hard projection，不扩频、不加噪声、不做 cross-power。这只
拒绝 flat-prior realization，不拒绝所有无模板 sky-smooth 模型。下一候选应使用有限
sky-smooth covariance 或保守 avoidance limit；slope/curvature variance 只能由
control/guard data 选择，并继续使用 exact matrix-free operator actions 与显式
signal-transfer/window gates。不得恢复 fixed morphology template。实现和结果位于
`3dnet/ps2d_v2_sky_smooth.py`、
`3dnet/ops_scripts/evaluate_template_free_sky_smooth_projection.py` 与
`runs/template_free_sky_smooth_20260713/`。

### 13.7 无模板有限协方差 MAP screen

13.6 之后实现了最小有限协方差版本，仍不使用 foreground morphology/reference template：

$$
\min_a\;\|F_{\rm control}(BQa-d)\|_2^2
+\lambda\sum_m\|a_m/\tau_m\|_2^2.
$$

每个 $a_m$ 仍是自由 `512×512` map；第二项等价于零均值、逐像素各向同性的有限 Gaussian
covariance，不固定空间形态。51,431 个复数 native-$k_\parallel$ index-0 modes 只用于
训练，102,862 个互斥 index-1 modes 只用于超参数选择；science modes 两阶段都不可见。
完整 `sky -> cached B -> taper/demean -> FFT -> control` adjoint error 最大为
`1.55e-15`。12 次 LSQR 被冻结为 screen estimator 的一部分。

T0/T1 等方差 ridge scan 的 guard-only 结果为：

| ridge fraction | `1e-8` | `1e-4` | `1` | `1e2` | `1e3` | `1e4` |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| guard residual | 0.0543986 | 0.0543986 | 0.0544118 | 0.0602718 | 0.148693 | 0.458009 |

最低两点的 score 相对差仅 `2.33e-8`，低于 `1e-6` numerical plateau stop；因此 selector
选择的是近似 flat 的低-ridge 极限，有限 covariance 本身没有 held-out 改善。ridge=1 下，
`tau1/tau0=1/0.3/0.1/0.03` 的 guard scores 为
`0.0544118/0.0544309/0.0544386/0.0544395`，数据不支持压缩斜率方差。加入自由 T2 map，
`tau2/tau0=1/0.3/0.1/0.03` 得到
`0.0997902/0.0556316/0.0544600/0.0544111`；T2 只在方差趋零时回到 T0/T1 极限。

冻结入选项为 T0/T1、`tau=[1,1]`、ridge `1e-8`、12 iter：

| metric | selected | non-selected ridge-100 diagnostic |
| --- | ---: | ---: |
| control / guard residual | `0.046614 / 0.054399` | `0.052190 / 0.060272` |
| observed target L2 | `9.851e5` | `9.548e5` |
| foreground residual / EoR target power | `1.425e6` | `1.398e6` |
| pure-EoR target L2 / power ratio | `0.05950 / 0.94533` | `0.05619 / 0.94838` |
| pure-EoR max per-band error | `0.05958` | `0.05627` |

pure-EoR transfer 尚可，但前景残留仍高约六个数量级；更强 ridge 没有形成可用折中。因此
停止 control-trained finite-Gaussian MAP subtraction，不扩频、不加噪声、不做 cross-power。
这不否定所有无模板 covariance model。后续若继续，应在 bandpower likelihood 中
marginalize nuisance covariance，或采用保守 avoidance，而不是构造并扣除单个 MAP
foreground cube。实现和结果：
