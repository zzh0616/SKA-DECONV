...
状态：方法设计与测试计划。本文档不表示该估计器已经通过下述科学验收。

## 1. 结论摘要

目前发现的不可识别区域确实与正在使用的 EoR window 有重叠，但并不覆盖整个
EoR window。

重叠最严重的位置是最低的横向波数，以及刚进入 EoR window 的第一个视向波数
频带。当前 window 从
$k_\parallel=0.1356\ {\rm Mpc}^{-1}$ 开始，第一个有效 bin 的中心约为
$0.153\ {\rm Mpc}^{-1}$；现有实验中 compiled foreground response 与 dirty EoR
response 的退化恰好集中在这里。随着 $k_\parallel$ 增大，可识别区域会逐渐向更高
...
32 arcsec 空间像素后，第一个 EoR-window band 附近的严格可恢复区域约为
...
横向内切圆 mask 和 DC exclusion 必须与现有 dirty-EoR evaluator 完全一致。
...
$k_\perp$ 和第一个 EoR-window band，REML/truth 为 16.78。下一条
$k_\parallel=0.25527\ {\rm Mpc}^{-1}$ 的 low-$k_\perp$ band 仍为 2.13。
...
当前配置使用 $k_\parallel\ge0.1356\ {\rm Mpc}^{-1}$、wedge slope 0，因此包含二者。
标准 wedge 边界随 $k_\perp$ 增长，而不是按 EoR 功率大小切除。119.3 MHz 下 horizon
slope 约为 4.270；若对无 PB/isobeam 设置采用保守 horizon profile，8wide target 的
geometric window 只剩最低 $k_\perp$ 的这两个 bins，而 correction support 又排除二者，
交集为空。beam、horizon 和 supra-horizon buffer 未冻结前，slope-0 mask 只能称为简化
evaluation window。
